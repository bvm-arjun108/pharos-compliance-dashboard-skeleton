...
final class ReportGroupConfigNativeQueries {
  ...
  private static final String FILTERED_LATEST_CONFIGS =
      ...
      WITH ranked_configs AS (
          ...
          WHERE (:country = 'ALL' OR UPPER(BTRIM(country_code)) = :country)
            AND (:status = 'ALL'
                 OR (:status = 'ACTIVE' AND rpt_config_active_flag IS TRUE)
                 OR (:status = 'INACTIVE' AND rpt_config_active_flag IS NOT TRUE))
  ...
  static final String SUMMARY =
      FILTERED_LATEST_CONFIGS
          ...
          SELECT
              ...
              COUNT(*) FILTER (WHERE rpt_config_active_flag IS TRUE) AS "activeConfigurations",
  ...
  static final String LIST =
      FILTERED_LATEST_CONFIGS
          ...
          FROM filtered_configs
          ORDER BY
              CASE WHEN rpt_config_active_flag IS TRUE THEN 0 ELSE 1 END,
