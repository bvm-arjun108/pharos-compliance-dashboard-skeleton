...
@Tag(
    ...
    description = "Read-only report-group configuration discovery and inspection")
@RequestMapping("/api/v1/report-configs")
public interface ReportConfigApi {

  @Operation(
      operationId = "getReportConfigFilterOptions",
      summary = "Get report-configuration filters",
      description = "Returns distinct active countries and report types from report_group_config.")
  @ApiResponse(
      responseCode = "200",
  ...
  @Operation(
      ...
      summary = "Explore latest report-group configurations",
      description =
          "Returns the latest configuration version for each report group, filtered by country, status, report type, or exact report group ID.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
        description = "Report configurations returned successfully",
        headers = {
          @Header(name = "X-Trace-Id", description = "Distributed trace identifier"),
          @Header(name = "X-Span-Id", description = "Current server span identifier")
        },
        content = @Content(schema = @Schema(implementation = ReportConfigExplorerResponse.class))),
    @ApiResponse(
        responseCode = "400",
        description = "A supplied filter is invalid",
  ...
  ReportConfigExplorerResponse getReportConfigs(
      @Parameter(description = "Active report_group_config country code or ALL", example = "SG")
          ...
          String country,
      @RequestParam(value = "status", defaultValue = "ALL") ReportConfigStatus status,
  ...
  @Operation(
      operationId = "getReportConfigDetails",
      summary = "Get one complete report-group configuration",
      description = "Uses the full report-group configuration composite identity.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "200",
        description = "Configuration returned successfully",
        content = @Content(schema = @Schema(implementation = ReportConfigDetailsResponse.class))),
    @ApiResponse(
        responseCode = "404",
