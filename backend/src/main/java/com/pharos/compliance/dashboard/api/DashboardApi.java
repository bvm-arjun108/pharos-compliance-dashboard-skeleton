...
public interface DashboardApi {
  @Operation(operationId = "getDashboardDetails", summary = "Get dashboard details", description = "Returns aggregate batch KPIs, "
      + "adaptive health trends, issue-driver trends, and report groups requiring attention for an inclusive date range.")
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Dashboard details calculated successfully", headers = {@Header(name = "X"
      + "-Trace-Id", description = TRACE_ID_DESCRIPTION), @Header(name = SPAN_ID_HEADER, description = SPAN_ID_DESCRIPTION)}, content = @Content(schema = @Schema(implementation = DashboardDetailsResponse.class))),
